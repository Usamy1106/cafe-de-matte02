...
const App = () => {
    ...
    const handleShareService = () => {
        const text = "遅刻したときの強い味方！待っている相手にコーヒーを贈ろう☕「café de matté」";
        const url = window.location.href.split('?')[0]; // Remove query params
        const hashtags = "cafedematte,遅刻,カフェ";
                ...
                <div className="step1-container">
                    <div className="logo-header" style={{
                        ...
                        <div className="header-actions">
                            <button className="header-btn" onClick={handleShareService} aria-label="Share on X">
                                <XLogo />
                            </button>
                            <button className="header-btn" onClick={() => { setHelpStep(0); setHelpOpen(true); }} aria-label="Help">
                        ...
                        <button className="btn-primary w-full" onClick={async () => {
                            if (navigator.share) {
                                try {
                                    ...
                                    console.debug('Share failed or canceled', error);
                                }
                            } else {
                                handleCopy();
                                alert('リンクをコピーしました');
